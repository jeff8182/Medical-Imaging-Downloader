...
class PAC:
    ...
    def vr_anon_callback(self, dataset, data_element):
        ...
        if vr in self.ANON_VRS:
            try:
                data_element.value = self.VR_ANON_MASKS[vr]
            # we should never reach this exception, but if the mask doesn't work for whatever reason, just default to
            # an empty string. That should work for most of the standard VR types
    ...
    def anonymize_dataset(self, ds, anon_studyUID):
        ...
        for vr, tagnames in self.ANON_TAGS_MINUS_ANON_VRS.items():
            for tagname in tagnames:
                if tagname in ds:
                    ds.data_element(tagname).value = self.VR_ANON_MASKS[vr]
