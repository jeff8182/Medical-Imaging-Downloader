...
class Main:
    ...
    def establish_numbering(self, df, ordering, numbering_col='Study', unique_col='StudyInstanceUID',
        ...
        df_index_by_study.index += numbering_start
    ...
    def apply_dual_selections(self, df, dual_selections, all_prefix, ignore_independent_vals=[],
                              reestablish_numbering=False, numbering_start=1, ordering=None, numbering_col='Study', \
        ...
        if reestablish_numbering:
            df = self.establish_numbering(df, ordering, numbering_col, unique_col, numbering_start)
    ...
    def run(self):
        ...
        while True:
            ...
            # DEPRECATED, NOT USED
                ...
                ui.set_phase(ui.PHASE_MOVE)
            elif event == '_STUDYOFFSET_MAIN_':

                temp_numbering_start = values['_STUDYOFFSET_MAIN_']

                # allow the field to be blank, to facilitate easier number entry (if user leaves it completely blank,
                # will default to 1)
                if not temp_numbering_start:
                    numbering_start = 1
                else:
                    try:
                        # Parse the updated field to an int
                        num = int(re.search('\d+', temp_numbering_start).group())
                        numbering_start = num
                    except Exception as err:
                        # if this doesn't work, then just default back to the existing valid integer value
                        pass

                    ui.set_input(ui.main_window, '_STUDYOFFSET_MAIN_', numbering_start)
