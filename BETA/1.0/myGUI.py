...
class GUI:
    ...
    def set_combo(self, window, key, lst, select=None):
        window.Element(key).Update(select, lst)
    ...
    def popupSelector(self,
        ...
        while True:                 # Event Loop
            ...
            if highlight_available is not None:
                ui.set_listbox_highlight(selector_window, '_LST_ALL_SEL_', highlight_available)
            ...
            if highlight_selected is not None:
                ui.set_listbox_highlight(selector_window, '_LST_SEL_SEL_', highlight_selected)
    ...
    def initializePopupAdvancedSelector(self,
        ...
        advanced_selector_window = sg.Window(
            title=title,
            layout=self.advanced_selector_layout,
            keep_on_top=keep_on_top
        ).Finalize()
    ...
    def popupDataFrameDualSelector(self,
        ...
        # --- All variable names
        ALL = '<ALL>'
        variables = [ALL] + df.columns.values.tolist()

        # --- INDEPENDENT and DEPENDENT variable selection
        if independent_default is None:
            independent_default = variables[0]
        if dependent_default is None:
            dependent_default = variables[1]

        # --- values for the selected INDEPENDENT variable
        independent_available = [ALL] + list(df[independent_default].unique())
        independent_available_highlights = [independent_available[0]]

        # --- APPLY THE DUAL SELECTION FILTERS
        df_selected = apply_dual_selections(df, dual_selections)

        # --- values for the selected DEPENDENT variable
        dependent_available = []
        dependent_selected = list(df_selected[dependent_default].unique())

        # --- Labels
        if independent_txt is None:
            independent_txt = 'Selected %s Values' % independent_default
        if txt_available is None:
            txt_available = 'Available %s Values' % dependent_default
        if txt_selected is None:
            txt_selected = 'Selected %s Values' % dependent_default
        ...
        advanced_selector_window = self.initializePopupAdvancedSelector(
            ...
            dependent_default=dependent_default,
        ...
        while True:
            ...
            if sort_available:
                temp_available = sorted(temp_available)
            if update_available:
                ui.set_listbox(advanced_selector_window, '_LST_ALL_SEL_', temp_available)
            if highlight_available is not None:
                ui.set_listbox_highlight(advanced_selector_window, '_LST_ALL_SEL_', highlight_available)

            # 'selected' list has user-defined order
            if sort_selected:
                temp_selected = sorted(temp_selected)
            if update_selected:
                ui.set_listbox(advanced_selector_window, '_LST_SEL_SEL_', temp_selected)
            if highlight_selected is not None:
                ui.set_listbox_highlight(advanced_selector_window, '_LST_SEL_SEL_', highlight_selected)
    ...
    def createMainWindow(self,
        ...
        combo_headings_main = sg.Combo(headings_pretty_results_main,
                                       default_value=default_filter_main,
                                       size=(21, 1),
                                       readonly=True,
                                       key='_COMBO_FILT_MAIN_')
        btn_value_selector = sg.Button('3. SELECT VALUES',
                                       size=(20, 1),
                                       button_color=self.default_btn_color,
                                       disabled=True,
                                       key='_FILT_MAIN_')
    ...
    def createUI(self,
        ...
        # *******************************
        self.selector_layout = self.createSelectorLayout()
        self.advanced_selector_layout = self.createAdvancedSelectorLayout()
