...
import numpy as np
# --- These 3 imports are needed for PyInstaller to work properly with pandas, because they somehow get dropped from
# numpy in the conversion process. Jank.
...
class Main:
    ...
    # processing done when we initially load an excel file
    parse_excel_func = {}
    parse_val_to_tag_func = {}
    parse_tag_to_val = {}
    ...
    DEFAULT_USER_DEFAULTS = {
        ...
        "default_anon_prefix": "HIPAA_",
        "default_raw_prefix": "RAW_",
        "default_search_results_suffix": "_0_SEARCH_RESULTS",
        "default_success_suffix": "_2_SUCCESSFUL_DOWNLOADS",
        "default_ignored_suffix": "_3_IGNORED_FOR_HIPAA",
        "default_deselected_suffix": "_4_USER_EXCLUDED",
        "default_failed_suffix": "_5_FAILED_TRANSFERS",
        "default_missing_suffix": "_6_QUERY_NOT_FOUND",
        "default_legend_suffix": "_1_MASTER_KEY_LEGEND",
    ...
    # --- Formatting conversions (dataframe value <-> DICOM Tag value)
    ...
    # MRN conversions (ensure that leading zeros are included)
    ...
    def craft_queries(self, df, query_tags, query_identifier_col='Query', args=None):
        queries = []
        ...
        for _, row in df.iterrows():
            ...
            for tag in query_tags:
                ...
                q[tag] = self.parse_val_to_tag_func[tag](val, args) if tag in self.parse_val_to_tag_func else val
    ...
    def format_df_general(self, df):
        ...
        return df
    def format_df_columns(self, df, parse_func={}):
        for heading in df.columns:
            if heading in parse_func:
                df.loc[:, heading] = \
                    df[heading].map(lambda x: parse_func[heading](x, None))
        return df
    def format_df(self, df, parse_func={}):
        # --------- General formatting
        df = self.format_df_general(df)

        # --- format individual columns
        df = self.format_df_columns(df, parse_func)
    ...
    def apply_dual_selections(self, df, dual_selections):
        pass
    ...
    def run(self):
        ...
        anon_prefix = user_defaults['default_anon_prefix']
        raw_prefix = user_defaults['default_raw_prefix']
        search_results_suffix = user_defaults['default_search_results_suffix']
        success_suffix = user_defaults['default_success_suffix']
        ignored_suffix = user_defaults['default_ignored_suffix']
        deselected_suffix = user_defaults['default_deselected_suffix']
        missing_suffix = user_defaults['default_missing_suffix']
        failed_suffix = user_defaults['default_failed_suffix']
        legend_suffix = user_defaults['default_legend_suffix']
        ...
        while True:
            ...
            if key is not None:
                if key == '_T_ERROR_':
                    ...
                    try:
                        df_results_temp = self.format_df(df_results_temp, self.parse_tag_to_val_func)
                    except Exception as err:
                        traceback.print_stack()
                    ...
                    # should stay consistent. The indexes are established after sorting the by the preestablished sorted query
                    # column order
                    df_results = df_results.sort_values(by=query_table_headings)
                    df_results['tempID'] = df_results.index
                    # --- select only 1 of each studyinstanceuid, but also keep all rows that have studyinstanceuid==''
                    df_index_by_study = df_results.loc[
                        (~df_results['StudyInstanceUID'].duplicated()) |
                        (df_results['StudyInstanceUID'] == ''),
                        ['tempID', 'StudyInstanceUID']
                    ]
                    # --- set the indexing based on this
                    df_index_by_study = df_index_by_study.reset_index(drop=True)
                    df_index_by_study.index += 1
                    df_index_by_study['Study'] = df_index_by_study.index
                    # --- convert the ints into 4-digit integer strings
                    df_index_by_study.loc[:, 'Study'] = \
                        df_index_by_study.loc[:, 'Study'].apply(lambda x: self.parse_pad_num(x))
                    # --- Apply our established indexes to the main results dataframe
                    df_results = df_results.merge(df_index_by_study, on=['tempID', 'StudyInstanceUID'], how='outer')
                    df_results = df_results.drop(columns=['tempID'])
                    ...
                    df_results = df_results.fillna(method='ffill')
                    ...
                    df_master_out = df_results[headings_out]
                    fname_out = basename_out + legend_suffix + '.xlsx'
                    path_out = os.path.join(dir_out, fname_out)
                    self.save_to_xlsx(ui, path_out, df_master_out, file_descriptor='MASTER KEY/LEGEND')

                    # --- IGNORED entries (primarily due to ANONYMIZATION)
                    df_ignored_out = df_results[df_results['Status'].str.startswith('IGNORED')][headings_out]
                    fname_out = basename_out + ignored_suffix + '.xlsx'
                    path_out = os.path.join(dir_out, fname_out)
                    self.save_to_xlsx(ui, path_out, df_ignored_out, file_descriptor='ANON-IGNORED ENTRIES')

                    # --- DESELECTED/EXCLUDED entries (due to manual user selection/deselection)
                    df_deselected_out = df_results[df_results['Status'].str.startswith('READY')][headings_out]
                    fname_out = basename_out + deselected_suffix + '.xlsx'
                    path_out = os.path.join(dir_out, fname_out)
                    self.save_to_xlsx(ui, path_out, df_deselected_out, file_descriptor='USER-DESELECTED ENTRIES')

                    # --- MISSING entries (not found in the peer's database)
                    df_missing_out = df_results[df_results['Status'].str.startswith('MISSING')][headings_out]
                    fname_out = basename_out + missing_suffix + '.xlsx'
                    path_out = os.path.join(dir_out, fname_out)
                    self.save_to_xlsx(ui, path_out, df_missing_out, file_descriptor='MISSING ENTRIES')

                    # --- FAILED entries (found in the peer's database but either transfer or local storage failed)
                    df_failed_out = df_results[df_results['Status'].str.startswith('FAILED')][headings_out]
                    fname_out = basename_out + failed_suffix + '.xlsx'
                    path_out = os.path.join(dir_out, fname_out)
                    self.save_to_xlsx(ui, path_out, df_failed_out, file_descriptor='FAILED ENTRIES')

                    # --- SUCCESSFUL entries
                    df_success_out = df_results[headings_out]
                    df_success_out = df_success_out.merge(
                        pd.concat([df_ignored_out, df_deselected_out, df_missing_out, df_failed_out]),
                        how='left',
                        on=df_success_out.columns.values.tolist(),
                        indicator=True
                    )
                    df_success_out = df_success_out[df_success_out['_merge'] == 'left_only'].drop(columns=['_merge'])
                    fname_out = basename_out + success_suffix + '.xlsx'
                    path_out = os.path.join(dir_out, fname_out)
                    self.save_to_xlsx(ui, path_out, df_success_out, file_descriptor='SUCCESSFUL ENTRIES')

                    # 8. --- Update our table with all the move results
                    if sort_yes:
                        df_results = df_results.sort_values(by=sort_yes)
                    tbl_studies = self.apply_selections(df_results, selections).values.tolist()
            ...
            if event == '':
                ...
                # --- set query number, just to help associate each query with its results
                ...
                try:
                    df_queries_temp = self.format_df(df_queries_temp, self.parse_excel_func)
                except Exception as err:
                    traceback.print_stack()
                ...
                # --- standardize excel values to dataframe values as necessary
                try:
                    df_results_temp = self.format_df(df_results_temp, self.parse_excel_func)
                except Exception as err:
                    traceback.print_stack()
                    ui.popupError('***ERROR***\nFailed to load file \'%s\'\n%s: %s' %
                                  (os.path.basename(load_path), type(err).__name__, err))
                ...
                queries = self.craft_queries(df_queries, query_tags=query_tags_sorted, args=args)
            ...
            # --- User-selected header tag filters for which studies/series we ultimately download
            elif event == '_DUAL_FILT_MAIN_':

                if df_results is None or len(df_results) <= 0:
                    continue

                dual_selections = ui.popupDataFrameDualSelector(ui,
                                              df_results,
                                              self.apply_dual_selections,
                                              dual_selections,
                                              independent_default='StudyDescription',
                                              dependent_default='SeriesDescription')


                selected_heading = values['_COMBO_FILT_MAIN_']
                ...
                df_move = self.apply_selections(df_move, selections)
                ...
                # assemble queries
                queries = self.craft_queries(df_move, query_tags=query_tags_sorted, args=None)
